      ...
      <div className="relative z-10 container mx-auto px-2 text-center text-white">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            {/* CTA Buttons */}
            <div
              className="flex flex-col sm:flex-row gap-3 justify-center items-center mb-8 animate-enter"
              style={{ animationDelay: '0.7s', animationFillMode: 'both' }}
            >
              <Button
                size="lg"
                className="btn-industrial text-base px-6 py-3 glow-effect"
              ...
              <Button
                size="lg"
                variant="outline"
                className="glass-morphism border-white/30 text-base py-3 bg-slate-200 hover:bg-slate-100 text-slate-900 px-[40px]"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              {t.stats.map((stat, index) => (
                <div
                  key={index}
                  className="metric-display rounded-lg p-4 glass-morphism bg-slate-800 animate-enter"
                  style={{ animationDelay: `${0.9 + index * 0.1}s`, animationFillMode: 'both' }}
                >
                  <div className="text-2xl md:text-3xl font-bold text-primary mb-1">{stat.number}</div>
                  <div className="text-xs md:text-sm text-gray-300 bg-blue-500">{stat.label}</div>
                </div>
              ))}
            </div>
